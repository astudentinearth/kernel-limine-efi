...
void init_pmm()
{
    ...
    if (max_available_pages > MAX_PAGE_COUNT) {
        max_available_pages = MAX_PAGE_COUNT;
    }
    pool_ptr = pool_start;
...
void test_allocator()
{
    ...
    describe("pool alignment check",
             assert_equals_uint(0, pool_ptr % PAGE_SIZE,
                                "is page pool start aligned?"),
             assert_equals_uint(0, pool_end_ptr % PAGE_SIZE,
    ...
    describe("used frame check", assert(frame_map[i1] == USED, "frame 1 used"),
    ...
    describe("free frame check", assert(frame_map[i1] == FREE, "frame 1 free"),
